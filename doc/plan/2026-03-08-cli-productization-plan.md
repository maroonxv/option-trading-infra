...
```bash
option-scaffold init my_strategy
option-scaffold init iron_condor --destination example
option-scaffold init ema_breakout --force
```

### 5.2 `option-scaffold run`
...
建议新增一个真正的 Python 包作为对外 CLI 入口，而不是继续依赖 `scripts/` 下的桥接脚本。
...
- `init` -> 调用 `src/main/scaffold/cli.py`
...
```toml
[project.scripts]
option-scaffold = "option_strategy_scaffold.cli:app"
```
...
- `pip install option-strategy-scaffold`
- `pipx install option-strategy-scaffold`
...
2. 新增 `pyproject.toml`，暴露 `option-scaffold` 命令
